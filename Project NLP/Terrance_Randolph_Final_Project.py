...
if __name__ == '__main__':
    ...
    freqFilter_Raw(processkaggle_s2(txt,lmt))
    freqFilter_Pmi(processkaggle_s2(txt,lmt))
    MostCommon(processkaggle_s2(txt,lmt))

    # # ------------------------------------------------------- #
    print("\n"+" --- NON ALPHA EXAMPLE AFTER --- ")
    for char in non_alpha_filter(processkaggle_s2(txt,lmt)):
      print(char)
    print("\n"+" --- STOPWORDS NEGATION EXAMPLE AFTER --- ")
    for negate in sw_negation(processkaggle_s2(txt,lmt)):
      print(negate)
    print("\n"+" --- STEMMER LANCASTER EXAMPLE AFTER --- ")
    for stemed in stem_lan(processkaggle_s2(txt,lmt)):
      print(stemed)
